...
let readAdmins  = fs.readFileSync("admin.txt", "utf8")
let ADMINS = JSON.parse(readAdmins);

let readUsers  = fs.readFileSync("user.txt", "utf8")
let USERS = JSON.parse(readUsers);

let readCourses  = fs.readFileSync("course.txt", "utf8")
let COURSES = JSON.parse(readCourses);
...
app.post('/admin/signup', (req, res) => {
  ...
  if(admin) {
    ...
    console.log("new admins" + ADMINS)
  
    fs.writeFileSync("admin.txt",JSON.stringify(ADMINS));
